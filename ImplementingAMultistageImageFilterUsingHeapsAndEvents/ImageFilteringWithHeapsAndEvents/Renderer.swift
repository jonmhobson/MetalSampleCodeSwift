...
final class Renderer: NSObject {
    ...
    private let event: EventWrapper
    ...
    init(metalView: MTKView) {
        ...
        self.event = EventWrapper(device: device)
        ...
        if let commandBuffer = commandQueue.makeCommandBuffer(), let blitEncoder = commandBuffer.makeBlitCommandEncoder() {
            ...
            event.signal(commandBuffer: commandBuffer)
    ...
    func executeFilterGraph(inTexture: MTLTexture) -> MTLTexture {
        ...
        resultTexture = downsample.execute(commandBuffer: commandBuffer, inTexture: inTexture, heap: scratchHeap, event: event)
        resultTexture = gaussianBlur.execute(commandBuffer: commandBuffer, inTexture: resultTexture!, heap: scratchHeap, event: event)
...
extension Renderer: MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        event.wait(commandBuffer: commandBuffer)
